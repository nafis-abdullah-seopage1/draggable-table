        ...
        <div className="p-3 flex justify-end">
            <details className="dropdown dropdown-end w-40">
                ...
                <ul className="p-2 shadow menu dropdown-content z-[1] flex-nowrap rounded bg-white border-2 w-52 space-y-1 max-h-[50vh] overflow-y-auto">
                    {
                        [...columns].map((col, i) => <ColIndex key={i} col={col} i={i} setColsList={setColsList} colsList={colsList} columns={columns} />)
                    }
...
const ColIndex = ({col,i,setColsList,colsList,columns}) => {
    const [checked, setCheck] = useState(true);

    useEffect(() => {
        if (checked) {
                const newList = [...colsList, col];
